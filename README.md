...
- 관리자가 화면에서 붙이는 방문 추적(Momento 우선, nonce 기반 CSP, 차단 출처 기록)
- 사내 SMTP 릴레이로 보내는 이벤트 메일(자리 배정·분석 완료·인사 연동 실패·API 키 만료 임박, 배경 발송과 발송 기록)
...
관리자 → 시스템 설정 → 방문 추적에서 화면을 다시 배포하지 않고 추적 도구를 붙인다. 기본값은 꺼짐이다. 추적 도구는 사내 자체 호스팅 수집기 **Momento**(기본, 같은 오리진 프록시 `/momento/*`로 외부 출처 없이 동작)와 GA4·GTM·Matomo·직접 붙여넣기(8KB)를 지원한다.

SeatOn의 응답은 `script-src 'self'` 로 잠겨 있으므로 스니펫을 그냥 넣으면 브라우저가 조용히 막는다. 추적이 켜진 문서에는 요청마다 다른 nonce 를 스니펫의 모든 `<script>` 와 `script-src` 에 함께 넣고, 스니펫에 적힌 출처를 정책에 더하며, 브라우저가 차단한 출처를 `report-uri` 로 받아 같은 화면에서 한 번 눌러 허용에 더할 수 있다. `'unsafe-inline'` 은 쓰지 않고, 끄면 정책이 원래대로 돌아간다. 자세한 설정과 CSP 설명은 [docs/ADMIN_GUIDE.md](docs/ADMIN_GUIDE.md) §3.6 에 있다.

## 메일 알림

관리자 → 시스템 설정 → 메일 알림에서 사내 SMTP 릴레이를 가리키면 사람이 실제로 기다리는 네 가지 — **자리가 정해짐**(그 직원), **도면 분석 완료·실패**(요청한 관리자), **예약 인사 연동 실패**(시스템 관리자), **API 키 만료 7일 전**(키 소유자) — 를 메일로 보낸다. 기본값은 꺼짐이며 사내 릴레이의 흔한 모양(25번 포트·인증 없음·TLS 없음)이 기본값이라 주소만 넣고 켜면 된다. 폐쇄망이면 릴레이로 사내 메일 서비스 `postra` 를 가리킨다.

메일은 요청을 막지 않고 배경에서 나가며, 릴레이가 죽어 있어도 좌석 배정은 평소처럼 끝난다. 시도마다 발송 기록(제목·수신자·결과, 본문 없음)을 남기고, 자기가 한 일은 자기에게 보내지 않으며, SMTP 비밀번호는 저장 뒤 되읽히지 않는다. **저장 후 시험 발송**으로 릴레이 설정을 그 자리에서 확인한다. 설정 키와 이벤트 표는 [docs/ADMIN_GUIDE.md](docs/ADMIN_GUIDE.md) §3.7 에 있다.
...
node ../aidev/tools/guide/md2pdf.mjs docs/USER_GUIDE.md docs/USER_GUIDE.pdf \
  --title "사용자 가이드" --subtitle "좌석맵 검색부터 도면 등록·배정·MCP 연동까지" --project SeatOn --version v1.4.4
node ../aidev/tools/guide/md2pdf.mjs docs/ADMIN_GUIDE.md docs/ADMIN_GUIDE.pdf \
  --title "관리자 가이드" --subtitle "설치·설정·계정·운영·장애 대응·보안" --project SeatOn --version v1.4.4
```

## 릴리스

`v1.4.4` 형태의 태그를 push하면 GitHub Actions가 `linux/amd64` 서비스 이미지를 `seaton:v1.4.4` 으로 빌드하고 `docker save` 결과를 `SeatOn-v1.4.4.tar.gz` 로 GitHub Release에 첨부한다. 런타임에는 레지스트리나 인터넷이 필요 없다.

이름 규칙은 이미지 `서비스명:v버전`, 배포 파일 `서비스명-v버전.tar.gz` 이다. 애플리케이션이 `/api/v1/version` 으로 알리는 버전 문자열은 `v` 없는 semver(`1.4.4`)를 그대로 쓴다.

로컬 검증은 다음과 같다.

```bash
./scripts/release-image.sh 1.4.4
gzip -t SeatOn-v1.4.4.tar.gz
```

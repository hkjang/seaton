...
주소 `/admin/settings`. 시스템 관리자만. 상단 칩이 현재 상태(`Keycloak 비활성`, `인사 동기화 비활성`, `오프라인 CV 엔진`, `방문 추적 비활성`, `메일 알림 비활성`)를 요약하고, 오른쪽 위에 저장 여부(`모든 변경 저장됨` / `저장하지 않은 변경`)가 표시됩니다. 비밀값은 저장 후 `********`로만 보이고, 빈 값이나 `********`로 다시 저장하면 기존 값을 유지합니다. 모든 저장은 감사 로그(`settings.update`)에 바뀐 키 이름과 함께 남습니다.
...
| 키 | 화면 항목 | 기본값 | 설명 |
| --- | --- | --- | --- |
| `mail.enabled` | 메일 알림 사용 | `false` | 꺼짐이 기본. 새 설치에서는 아무것도 나가지 않음 |
| `mail.smtp_host` | SMTP 릴레이 주소 | 빈 값 | 사내 릴레이(또는 `postra`) 주소. 비어 있으면 켜도 보내지 않고 로그 `메일 설정이 모자라 알림을 보내지 않았습니다` |
| `mail.smtp_port` | 포트 | `25` | 사내 릴레이는 대개 25 |
| `mail.security` | 보안 협상 | `auto` | `auto`(서버가 STARTTLS 를 알리면 씀) · `none` · `starttls`(필수) · `tls`(465 처럼 바로 TLS). `auto` 에서 포트가 465 면 `tls` 로 다룸 |
| `mail.skip_tls_verify` | TLS 인증서 검증 건너뛰기 | `false` | 사내 인증서가 사설일 때만 |
| `mail.username` · `mail.password` | SMTP 사용자 이름 · 비밀번호 | 빈 값 | 인증 없는 릴레이가 흔하므로 **선택**. 비밀번호는 비밀값(암호화 저장)이며 설정 API 가 되돌려주지 않음 |
| `mail.from_address` | 보내는 주소 | 빈 값 | 비우면 `seaton@<릴레이 주소>` |
| `mail.from_name` | 보내는 이름 | `SeatOn` | |
| `mail.base_url` | 메일 속 링크 주소 | 빈 값 | 예 `https://seaton.company.intra`. 비우면 메일에 링크를 넣지 않음 |
| `mail.timeout_seconds` | 연결 제한 (초) | `10` | 연결·세션 한도. 배경 발송은 2회 시도 |
| `mail.notify_seat_assigned` | 자리가 정해짐 → 그 직원 | `true` | 이벤트별 스위치 |
| `mail.notify_analysis` | 도면 분석 완료·실패 → 요청한 관리자 | `true` | |
| `mail.notify_hr_sync` | 예약 인사 연동 실패 → 시스템 관리자 전원 | `true` | |
| `mail.notify_api_key_expiring` | API 키 만료 7일 전 → 키 소유자 | `true` | |
...
![시스템 설정 · 메일 알림 — 사용·인증서 검증 스위치, 보안 협상, 릴레이 주소·포트·보내는 사람·인증·링크 주소, 이벤트 네 가지 스위치, 시험 발송 칸, 발송 기록 표(보냄·실패와 이유)](assets/guide/admin-settings-mail.png)

**시스템 설정 → 메일 알림** 탭에서 사내 SMTP 릴레이로 이벤트 알림을 보내게 합니다. 기본값은 꺼짐이며, 켜지 않은 설치는 메일이 한 통도 나가지 않습니다. 사내 릴레이는 대개 **25번 포트 · 인증 없음 · TLS 없음**이라 릴레이 주소만 넣고 켜면 되고, 인증과 암호화는 릴레이가 요구할 때만 채웁니다(`auto` 는 서버가 STARTTLS 를 알리면 쓰고 아니면 평문으로 갑니다). 폐쇄망이면 릴레이 주소로 사내 메일 서비스 **postra** 를 가리키면 알림이 밖으로 나가지 않습니다.

**보내는 이벤트.** SeatOn 에서 사람이 실제로 기다리는 일만 골랐습니다 — 이 메일이 오지 않으면 누군가 손해를 보거나 화면을 계속 새로고침하는 것들입니다. 단순한 "무언가 바뀜" 은 보내지 않습니다.

| 이벤트 | 받는 사람 | 언제 | 왜 |
| --- | --- | --- | --- |
| `seat.assigned` 자리가 정해짐 | 그 직원(직원 명부의 메일 주소) | 배정·이동·일괄 등록·MCP `assign_seat` 로 **실제로 자리가 바뀌었을 때** | 이사 날 자기 자리를 몰라 좌석맵을 새로고침하거나 관리자에게 묻는 일이 사라짐. 같은 자리에 다시 배정한 것은 바뀐 게 아니라 보내지 않고, 일괄 등록에서 한 직원이 여러 번 나오면 마지막 자리로 한 통 |
| `analysis.finished` 도면 분석 완료·실패 | 분석을 요청한 관리자 | 분석 잡이 `completed` 또는 `failed` 로 끝났을 때(제한 시간·패닉으로 끝난 것 포함) | 비전 모델 판독은 몇 분씩 걸려 화면을 떠나기 마련이고, 실패로 멈춘 것은 더더욱 알아야 함 |
| `hr_sync.failed` 예약 인사 연동 실패 | 활성 시스템 관리자 전원 | `hr.schedule` 로 돈 동기화가 실패했을 때 | 새벽에 조용히 실패하면 명부가 낡은 채 며칠이 감. 화면에서 직접 누른 **저장 후 지금 동기화**는 결과가 그 자리에 보이므로 보내지 않음 |
| `api_key.expiring` API 키 만료 임박 | 키 소유자 | 만료 7일 안으로 들어온 개인 키를 30분마다 확인해 **사람마다 한 통**, 키마다 한 번 | 만료되면 그 키를 쓰는 연동이 멈춤. `api_keys.expiry_notified_at` 로 표시해 같은 키에 다시 보내지 않으며, 메일이 꺼져 있는 동안은 표시하지 않아 켠 뒤 첫 회차에 나감 |

받는 사람의 주소는 **새 명부를 만들지 않고** 계정(`users.email`, 없으면 연결된 직원의 주소)과 직원(`employees.email`) 표에서 그때 읽습니다. 주소가 없는 사람은 조용히 건너뜁니다. **자기가 한 일은 자기에게 보내지 않습니다** — 관리자가 자기 직원 레코드에 자리를 배정하면 계정과 직원 레코드의 id 가 달라도 주소가 같으므로 빠집니다. 비활성 계정과 퇴직 직원에게는 보내지 않습니다.

**요청을 막지 않습니다.** 알림은 요청이 끝난 뒤 배경에서 나가며 릴레이가 느리거나 죽어 있어도 좌석 배정은 평소처럼 `204` 로 끝납니다. 배경 발송은 2초 뒤 한 번 더 시도하고 실패하면 로그 `알림 메일 발송 실패` 를 남깁니다. 시험 발송만 결과를 기다려 그 자리에서 보여 줍니다.

**시험 발송.** 같은 탭 아래의 **저장 후 시험 발송**(`POST /api/v1/settings/mail/test`, `{"recipient": "..."}`, 비우면 내 계정 주소)은 저장한 설정으로 실제 한 통을 보내고 결과를 바로 보여 줍니다. 알림과 같은 길을 지나므로 여기서 되면 알림도 됩니다. 릴레이에 닿지 못하면 `502 mail_send_failed` 와 이유(`SMTP 연결 실패: …`, `RCPT TO 실패: …`, `STARTTLS 실패: …`, `SMTP 인증 실패: …`), 꺼져 있거나 설정이 모자라면 `400 mail_config_invalid` 입니다.

**발송 기록.** 시도마다 `mail_deliveries` 에 남습니다 — 언제, 어떤 이벤트로, 누구에게, 제목이 무엇이었고, 되었는지(`queued` → `sent` / `failed`, 시도 횟수, 실패 이유). 같은 탭의 **발송 기록** 표(`GET /api/v1/settings/mail/deliveries?status=&limit=`, 최근 50건과 상태별 건수)에서 봅니다. **본문은 기록하지 않습니다.** "안 왔다" 는 문의에는 여기서 그 사람 주소로 나간 기록과 실패 이유를 찾아 답합니다.

**비밀번호.** `mail.password` 는 비밀값이라 설정 API 가 `********` 와 `configured` 만 돌려주고, 바꿀 때만 새 값을 받으며, 로그 어디에도 적지 않습니다.
...
| 작업 | 주기 | 하는 일 |
| --- | --- | --- |
| 정리 | 30분 | 만료된 세션·OIDC state 삭제, 재시작으로 끊긴 분석 잡 정리, 만료 7일 전 API 키 소유자에게 메일(§3.7, 메일이 켜져 있을 때만) |
| 스케줄러 | 1분 | `hr.sync_enabled`가 `true`이고 `hr.schedule`이 되면 인사 동기화 실행. 실패하면 관리자에게 메일 |
| 기동 직후 | 1회 | `analyzing` 상태에 갇힌 도면 복구(로그 `중단된 분석 잡을 정리했습니다`), 만료 임박 키 확인 |
| 메일 발송 | 이벤트마다 | 요청과 분리된 고루틴에서 릴레이에 보냄. 실패 시 2초 뒤 1회 재시도 후 `mail_deliveries` 에 `failed` |
...
| 메일이 안 옴, 시험 발송이 `502 mail_send_failed` | 이유 문구: `SMTP 연결 실패`(주소·포트·방화벽), `STARTTLS 실패`/`x509`(사설 인증서 → `mail.skip_tls_verify` 또는 `mail.security=none`), `SMTP 인증 실패`(자격 증명, 또는 인증 없는 릴레이면 사용자 이름을 비움), `RCPT TO 실패`(릴레이가 그 주소나 SeatOn 컨테이너의 중계를 거부) | 시스템 설정 → 메일 알림의 **발송 기록**과 로그 `알림 메일 발송 실패`. 릴레이 쪽에서는 SeatOn 컨테이너 주소의 중계 허용 여부 |
| 켰는데 아무것도 안 나가고 기록도 없음 | 로그 `메일 설정이 모자라 알림을 보내지 않았습니다`(호스트 없음 등), 그 이벤트의 `mail.notify_*` 스위치, 받는 사람의 계정·직원 레코드에 메일 주소가 있는지 | 설정 보완. 자기가 한 일은 자기에게 오지 않는 것이 정상 |
...
- **메일 알림**: `mail.password` 는 되읽히지 않고 로그에도 남지 않습니다. 발송 기록에는 제목과 수신자만 있고 본문이 없어 기록 자체가 유출 경로가 되지 않습니다. 릴레이는 사내 주소(`postra` 권장)만 가리키고, `mail.skip_tls_verify` 는 사설 인증서일 때만 켭니다.
- **인터넷 통신**: 기본 설정(`ai.engine=cv`, SSO·인사 연동·방문 추적·메일 알림 꺼짐)에서는 컨테이너가 밖으로 나가는 연결이 없습니다. 연결이 생기는 곳은 Keycloak Issuer, `hr.api_url`, `ai.vlm_base_url`, Momento 프록시를 켰을 때의 `tracking.momento_url`, 그리고 메일을 켰을 때의 `mail.smtp_host` 다섯 군데뿐이며 모두 사내 주소를 씁니다.

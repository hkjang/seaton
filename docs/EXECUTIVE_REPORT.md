...
| :--- | :--- | :--- | :--- |
| 가로 도면 (책상 30석) | 정상 판독 | **F1 1.000** · 검토 6건 | **F1 1.000** · 검토 0건 |
| PDF 도면 (책상 30석) | 좌석 오버레이 불가 | **F1 1.000** · 검토 0건 | **F1 1.000** · 검토 0건 |
| 세로 도면 (책상 28석) | 좌표 최대 40% 이탈 | **F1 1.000** · 검토 0건 | F1 0.933 |
...
- **검토 공수 감소**: 사내 비전 모델과 교차 검증한 좌석은 자동 승인 대상이 되어, 가로 도면에서 관리자 검토 항목이 6건에서 0건으로 줄었습니다.
...
판독 정확도와 별개로, **화면이 실제로 어떻게 그려지고 조작되는지**는 코드 검사로 확인할 수 없는 영역입니다. v1.2.0까지 발견된 결함 중 화면 이동·확대가 동작하지 않던 것, 좌석 클릭이 먹히지 않던 것은 모두 이 영역에서 나왔습니다.

v1.3.0부터는 릴리즈마다 실제 브라우저로 제품을 열어 확인합니다. 빌드한 이미지와 PostgreSQL을 띄우고, 빈 데이터베이스에 도면을 올려 실제 판독을 돌린 뒤, 로그인부터 좌석 배치 편집까지 28개 항목을 자동으로 밟습니다. 검증은 CI에서 모든 변경마다 실행되며 통과하지 못하면 병합되지 않습니다.

이 체계를 도입한 직후 다음이 드러나 함께 조치했습니다.

| 확인된 문제 | 영향 |
| --- | --- |
| 1280px 노트북 화면에서 좌석 상세 패널이 잘림 | 흔한 사무용 해상도에서 정보 일부에 접근 불가 |
| 네트워크 단절 시 "로그인 방식 없음"으로 안내하고 입력란을 감춤 | 관리자가 고칠 수 없는 설정을 확인하게 되고, 복구 후에도 재접속 불가 |
| 첫 실행 시 하지 않은 준비 단계까지 완료로 표시 | 도입 초기 관리자가 남은 작업을 인지하지 못함 |
| 조작 패널이 도면과 확대 배율 표시를 가림 | 좌석 확인·조작 방해 |
| 잘못 올린 도면 버전을 삭제할 수단 없음 | 오등록 시 복구 불가, 버전 누적 |

### 4.2 화면 검증으로 이어 확인한 범위 (v1.4.0)

검증 체계를 갖춘 뒤 남은 사각지대를 차례로 확인했습니다.

| 확인 대상 | 결과 |
| --- | --- |
| 도면 좌석의 키보드 접근 | 마우스로만 가능했음 → 방향키 탐색 도입 |
| 좌석 일괄 배정(CSV/XLSX) | 문서에는 있으나 화면에 없었음 → 화면 도입. 검토 중인 도면 버전에 잘못 배정되던 서버 결함도 함께 수정 |
| 500석 규모 도면 | 30석과 성능 차이 없음(60fps 유지). 조치 불필요 |
| 잘못 올린 도면 정리 | 그 층의 유일한 버전은 삭제 불가였음 → 게시 내림 도입 |

네 가지 중 셋은 결함이었고 하나(성능)는 확인만으로 끝났습니다. 확인 자체가 결론인 항목을 남겨 두는 것도 검증 체계의 몫입니다.

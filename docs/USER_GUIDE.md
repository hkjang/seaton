...
**키보드만으로 쓰기.** 도면 위 좌석은 그림이라 탭 순서에 들어오지 않습니다. 왼쪽 검색창에 이름·사번·조직을 넣고 Enter를 누르면 첫 결과의 좌석으로 확대되며 상세가 열립니다. 결과가 여럿이면 탭으로 목록을 옮겨 다니며 Enter나 Space로 고릅니다. 목록의 각 줄은 이름·소속·좌석 번호를 함께 읽어 주므로 화면 낭독기에서도 무엇을 여는지 알 수 있습니다.

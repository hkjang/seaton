...
- **게시 중인 도면**: 삭제 단추가 나타나지 않습니다. 지우면 좌석맵이 통째로 비기 때문입니다. **게시 내림**으로 먼저 내린 뒤 지우거나, 다른 버전을 게시해 밀어내십시오. 게시를 내리면 좌석맵에서는 사라지지만 좌석과 이력은 남으므로 언제든 다시 게시할 수 있습니다.
...
2. **키 만들기**를 누르고 이름과 범위(`read`, `write`, `mcp`)를 지정합니다. 범위를 벗어난 요청은 `403` 으로 막힙니다. 조회용으로 나눠 준 `read` 키로는 데이터를 바꿀 수 없고, `mcp` 범위가 없으면 MCP 연결도 되지 않습니다.
3. 발급 직후 화면에 한 번만 표시되는 `seat_...` 형식의 키 원문을 안전한 곳에 보관합니다. 서버는 해시만 저장하므로 원문은 다시 볼 수 없습니다.
4. 목록의 **상태** 칸이 사용 중 · 회전 유예 · 폐기됨을 알려 줍니다.
5. 키를 갈아 끼울 때는 **회전**을 씁니다. 새 키가 한 번 표시되고, 기존 키는 설정된 유예 시간(`security.rotation_grace_hours`, 기본 24시간) 동안만 함께 동작한 뒤 막힙니다. 연동을 새 키로 바꿀 시간을 벌어 줍니다.
6. 키가 유출된 것으로 보이면 **폐기**로 즉시 무효로 만듭니다. 유예 없이 그 순간부터 막히고 되돌릴 수 없으므로, 어떤 키인지 확인하는 창이 한 번 뜹니다.

...
func (s *Server) openAPI(w http.ResponseWriter, _ *http.Request) {
	writeJSON(w, 200, map[string]any{
		...
		"paths": map[string]any{
			...
			"/settings/mail/deliveries":       map[string]any{"get": operation("메일 알림 발송 기록 (status=queued|sent|failed, limit)", true)},
			"/settings/mail/test":             map[string]any{"post": operation("저장된 SMTP 설정으로 시험 메일 한 통 발송 ({recipient})", true)},
